...
void AAuraGameModeBase::SaveWorldState(UWorld* World, const FString& InDestinationAssetName) const
{
	...
	{
		if (InDestinationAssetName != FString(""))
		{
			SaveGame->MapAssetName = InDestinationAssetName;
			SaveGame->MapName = GetMapNameFromMapAssetName(InDestinationAssetName);
		}
		if (!SaveGame->HasMap(WorldName))
		{
			FSavedMap NewSaveMap;
			NewSaveMap.MapAsstName = WorldName;
		...
		{
			if (MapToReplace.MapAsstName == WorldName)
			{
...
void AAuraGameModeBase::LoadWorldState(UWorld* World) const
{
	...
	if (UGameplayStatics::DoesSaveGameExist(AuraGameInstance->LoadSlotName, AuraGameInstance->LoadSlotIndex))
	{
		...
		if (SaveGame == nullptr)
		{
			UE_LOG(LogAura, Error, TEXT("Faild to load slot"));
			return;
		}
